...
import { PROFILE_MENU } from "@/components/profile-menu";
...
export default function TopBarMobile() {
  ...
  const countUnreadNotifQuery = useQuery(
    [notificationService.countUnreadNotif.name],
    async () => {
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {countUnreadNotifQuery.data ? (
                <div className="absolute -top-1 -right-1 pointer-events-none w-4 h-4 rounded-full bg-gray-800 text-white flex items-center justify-center text-xs">
                  {countUnreadNotifQuery.data}
                </div>
              ) : null}
              <Bell size={20} />
            ...
            <button
              ...
              className="p-1 w-7 h-7 flex items-center justify-center rounded-full text-gray-500"
            >
