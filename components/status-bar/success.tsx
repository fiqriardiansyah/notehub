...
const Component = ({ icon, message, onClose }: { icon?: any; message: any; onClose?: () => void }) => {
  return (
    <motion.div className="flex px-3 items-center justify-between gap-3 py-2 bg-green-100 text-xs text-green-500 font-medium capitalize line-clamp-1 rounded">
      ...
      <button onClick={onClose} className="bg-transparent border-none text-gray-700 cursor-pointer">
        <X size={15} />
      </button>
    </motion.div>
  );
...
export const SucccessStatusBarAsProcess = ({ process }: { process: Process }) => {
...
export default function SucccessStatusBar(props: CommonState["statusBar"]) {
  ...
  if (props?.show && props?.type === "success") {
    return <Component message={props?.message} icon={props?.icon} onClose={onClose} />;
  }
  return null;
}
