        ...
        <DrawerContent className="z-50">
          ...
          <div className="container-custom flex flex-col gap-4">
            <div className="h-[250px] flex items-center justify-center overflow-hidden">
              {todo?.timer && (
                <Countdown endTime={todo?.timer?.endTime} startTime={todo?.timer?.startTime} onCompleteRender={onCompleteRender}>
                  {({ text, progress }) => (
            ...
            <div className="flex items-center justify-between w-full my-10 h-[40px]">
              {todo?.timer?.isEnd && <div>Task mark done at {moment(todo?.checkedAt).format("DD MMM, HH:mm")} 🎉</div>}
              {!todo?.timer?.isEnd && (
                <CheckboxCustom disabled={!!todo?.timer} checked={autoComplete} onChecked={setAutoComplete} label="Auto complete task" />
              )}
