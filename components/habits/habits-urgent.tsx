...
export default function HabitsUrgent({
  onChangeHabit,
  inPageHabits,
  className,
  ...props
  ...
  const getHabitsUrgent = useQuery(
    [habitsService.getUrgentHabit.name],
    async () => {
  ...
  const changeTodosMutate = useMutation(async (todos: Todo[]) => {
    return (await noteService.changeTodos({ noteId: habit!.id, todos })).data
      .data;
  });
  ...
  const nameOftheDay = () => {
    ...
    if (habit?.schedulerType === "weekly")
      return { name: inPageHabits ? "This Week" : "Weeks" };
    if (habit?.schedulerType === "monthly")
      return { name: inPageHabits ? "This Month" : "Months" };
  };
  ...
  const onCompleteTimer = (todo: Todo) => {
    if (todo?.timer?.autoComplete) {
      onSetTimer({
        ...
        checkedAt:
          todo?.checkedAt ||
          new Date(todo?.timer?.endTime || Date.now()).getTime(),
        timer: {
          ...todo?.timer,
          isEnd: true,
        },
    ...
    <div className="flex flex-col gap-1 overflow-hidden">
      {habit && (
        <div className="flex w-full justify-end">
          <Link href={`/habits/${habit?.id}`} passHref>
            {" "}
            <a className="text-gray-400 text-xs font-light">
              View Details
            </a>{" "}
          </Link>
        </div>
      ...
      >
        <div className="font-semibold z-10 leading-none">
          {!inPageHabits && (
            <p className="text-sm z-10 m-0">
              Habits on this{" "}
              <span className="text-xs font-light">{nameOftheDay()?.date}</span>{" "}
            </p>
          )}
          {inPageHabits && (
            <p className="text-sm z-10 m-0 line-clamp-1 flex flex-row">
          ...
          <div className="overflow-y-hidden h-[36px] flex-1 relative">
            <AnimatePresence mode="wait">
              ...
              {(progressDoneCheer === null ||
                progressDoneCheer === undefined) && (
                <motion.div
                  key="day"
                  initial={{ y: "40px" }}
                  animate={{ y: 0 }}
          ...
          {todos.map((todo) => (
            ...
            >
              <div className="flex gap-2 items-center h-[40px]">
                  ...
                  {(ctrl) => {
                    if (todo?.timer && !todo?.timer?.isEnd) {
                      return (
                        <Countdown
                          onCompleteRender={
                            ctrl.isOpen ? () => {} : () => onCompleteTimer(todo)
                          }
                ...
                <div className="">
                  <p className="text-sm m-0 capitalize leading-none line-clamp-1">
                    {todo.content}
                  </p>
                  {todo.isCheck && (
                    <span className="text-xs font-light m-0 leading-none">
                      done at {dayjs(todo.checkedAt).format("DD MMM, HH:mm")}
                    </span>
                  )}
                </div>
              </div>
        ...
        <div className="absolute bottom-0 left-0 w-full h-full flex flex-col justify-end">
          ...
          <motion.div
            ...
            transition={{ ease: easeDefault, delay: 0.4, duration: 0.8 }}
            className="transition duration-200 bg-[#d3d3ff] w-full"
          ></motion.div>
        </div>
      </div>
