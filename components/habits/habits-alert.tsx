...
export type HabitsAlertProps = {
  callback?: (note?: Note[]) => any;
};

export default function HabitsAlert({ callback }: HabitsAlertProps) {
  const router = useRouter();

  const habitsToday = useQuery(
    [habitsService.getUrgentHabit.name, "habitstoday"],
    async () => {
  ...
  const runningTimerQuery = useQuery(
    [habitsService.getRunningTimer.name],
    async () => {
  ...
  return (
    <>
      {!habitsToday.isLoading && callback && callback(habitsToday.data)}
      <div className="flex flex-col gap-4">
        <HabitsUrgent onChangeHabit={habitsToday.refetch} />
        <RunningTimer />
        <AnimatePresence>
          {habitsToday.data?.length &&
            habitsToday.data?.length > 1 &&
            !runningTimerQuery.data?.length && (
              <motion.button
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-semibold">
                    You have {habitsToday.data?.length} habits on going
                  </span>
                </div>
