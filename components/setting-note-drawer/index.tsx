...
export function withFunctionality(WrappedComponent: React.ComponentType<any>) {
  const Component = (props: any) => {
    ...
    const updateNoteMutate = useMutation(
      ...
      async (
        data: Partial<CreateNote> & { typeProcess: NoteSetting["type"] }
      ) => {
        setProcess({
          id: data.id!,
          nameOfProcess: "Updating " + data.title + "...",
        });
        await pause(1);
        const res = (await noteService.updateNote(data, currentNote!.id)).data
          .data;
    ...
    const deleteMutate = useMutation(async (note: Note) => {
      setProcess({
        id: note.id,
        nameOfProcess: "Deleting " + note.title + "...",
      });
      await pause(1);
    ...
    const { checkHasPassNote } = useSecureNote({
      ...
      onSecure(isPasswordCorrect) {
        if (isPasswordCorrect) {
          updateNoteMutate
            .mutateAsync({ isSecure: true, typeProcess: "secure_note" })
            .then(async () => {
              await queryClient.refetchQueries({
                queryKey: [noteService.getNote.name],
              });
    ...
    const handleClickSetting = (setting: NoteSetting) => {
      return () => {
        ...
        if (setting.type === "hang_note" || setting.type === "unhang_note") {
          ...
          updateNoteMutate
            .mutateAsync({ isHang: !currentHang, typeProcess: "hang_note" })
            .then(async () => {
              if (!currentHang) {
                setStatusBar({
                  autoClose: 5,
        ...
        if (setting.type === "remove_folder") {
          updateNoteMutate
            .mutateAsync({ folderId: "remove", typeProcess: "remove_folder" })
            .then(async () => {
              setNote((prev) => ({ ...prev, note: null }));
              generateChangesId();
            });
    ...
    const isLoading = (setting?: NoteSetting) => {
      ...
      if (setting.type === "hang_note" || setting.type === "unhang_note")
        return (
          updateNoteMutate.isLoading &&
          updateNoteMutate.variables?.typeProcess === "hang_note"
        );
      if (setting.type === "add_folder")
        return (
          updateNoteMutate.isLoading &&
          updateNoteMutate.variables?.typeProcess === "add_folder"
        );
      if (setting.type === "remove_folder")
        return (
          updateNoteMutate.isLoading &&
          updateNoteMutate.variables?.typeProcess === "remove_folder"
        );
      return false;
    };
