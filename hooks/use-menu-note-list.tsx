...
const selectSetting: NoteSetting = {
...
export default function useMenuNoteList(note?: Note | null) {
  ...
  React.useEffect(() => {
    ...
    if (!newSettings.find((s) => s.type === "select")) {
      newSettings.push(selectSetting);
    }
    ...
    if (
      !newSettings.find((s) => s.type === "remove_folder") &&
      note?.folderId
    ) {
    ...
    if (!newSettings.find((s) => s.type === "delete")) {
