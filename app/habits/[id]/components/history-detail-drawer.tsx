          ...
          <DrawerHeader>
            <DrawerTitle className="capitalize">
              ...
              {schedulerType === "weekly" &&
                `${moment.utc(history?.completedTime).startOf("week").format("dddd DD")} - ${moment
                  .utc(history?.completedTime)
                  .endOf("week")
          ...
          <div className="container-custom flex flex-col gap-4">
            ...
            <div className="w-full mb-20 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-2">
              {history?.todos?.map((todo) => (
                <ListCardHabit onClickAttach={onClickAttach} completedHabit={true} key={todo.id} todo={todo} />
              ))}
            </div>
          </div>
        </DrawerContent>
