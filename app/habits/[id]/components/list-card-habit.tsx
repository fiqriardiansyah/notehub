...
export default function ListCardHabit({
  todo,
  onCheck,
  progressDoneCheer,
  completedHabit,
  ...
  return (
    <div className="flex p-2 rounded-2xl flex-col border border-solid border-gray-200 relative overflow-hidden">
      {!completedHabit && (
        <AnimatePresence mode="wait">
          {progressCheer.map((pc, i) => {
            if (
              pc.donepoint === progressDoneCheer?.progress &&
              progressDoneCheer.todoId === todo.id
            ) {
              return (
                <motion.div
                  key={pc.donepoint}
      ...
      <div className="flex items-center justify-between">
        {!completedHabit ? (
          <Button
            ...
            size="icon-small"
            variant={todo?.isCheck ? "default" : "ghost"}
            className="mb-3"
          >
            <Check size={16} />
          </Button>
          ...
          <HabitsTimer anyId={noteId} setTimer={onSetTimer} todo={todo}>
            {(timerControl) => (
              <HabitsTodoDrawerMenu
                onClickTimer={timerControl.open}
                todo={todo}
                onAttachNote={onAddAttachNote}
              >
                {(drawerControl) => (
                    ...
                    {todo?.timer && !todo?.timer?.isEnd && (
                      ...
                      >
                        {({ text, progress }) => (
                          <button
                            onClick={
                              todo?.isCheck ? () => {} : timerControl.open
                            }
      ...
      <p
        onClick={onClickCheck(todo)}
        style={{ cursor: completedHabit ? "" : "pointer" }}
        className="m-0 capitalize font-medium"
      >
        {todo.content}
      </p>
      {todo.checkedAt && (
        <span
          onClick={onClickCheck(todo)}
          style={{ cursor: completedHabit ? "" : "pointer" }}
          className="m-0 text-xs leading-[10px] text-gray-400"
        >
          Done at {moment(todo.checkedAt).format("DD MMM, HH:mm")}
        </span>
      )}
      <div className="mt-1 flex flex-col gap-1">
        {todo?.attach?.map((attach) => (
          <a
            onClick={onClickAttachNote(attach)}
            key={attach.id}
            className="underline cursor-pointer line-clamp-1 text-[12px] font-medium flex items-center"
          >
