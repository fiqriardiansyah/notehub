...
export async function generateMetadata(
  ...
  return {
    ...
    openGraph: {
      images: [
        "https://ik.imagekit.io/p4ukigs1hrvx/portfolio?updatedAt=1696990893503",
        ...previousImages,
      ],
    },
  ...
  return (
    <div className="container-read text-justify mt-[10vh]">
      <div className="mih-h-[80vh] w-full">
        ...
        {content.type === "todolist" && (
          <TodoListView todos={content?.todos || []} />
        )}
      ...
      <div className="flex items-center gap-3 mt-10">
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="m-0 leading-none text-xs text-gray-500">{`Last update ${formatDate(
            content.updatedAt
          )} ${content.updatedBy ? `By ${content.updatedBy}` : ""}`}</p>
        </div>
      </div>
      {collaborators?.length ? (
        <div className="mt-5">
          ...
          <div className="flex items-center">
            <AnimatedTooltip
              size={40}
              items={collaborators as AnimatedTooltipItem[]}
            />
          </div>
        </div>
      ) : null}
