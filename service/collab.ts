...
import { CollabAccount, Invitation, InvitationData } from "@/models/collab";
...
const collabService = {
    ...
    cancelInvitation: (idInvitation: string): Promise<AxiosResponse<BaseResponse<boolean>>> =>
        api.delete(`collab/invite/${idInvitation}`),
    removeCollab: (idCollab: string): Promise<AxiosResponse<BaseResponse<boolean>>> =>
        api.delete(`collab/${idCollab}`),
    changeRoleCollab: (idCollab: string, role: string): Promise<AxiosResponse<BaseResponse<boolean>>> =>
